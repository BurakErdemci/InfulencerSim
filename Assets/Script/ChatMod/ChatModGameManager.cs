...
public class ChatModGameManager : MonoBehaviour
{
    ...
    void SpawnComment(float morality)
    {
        ...
        // Try to find a non-overlapping position (Overlap Fix)
        ...
        for(int i=0; i<10; i++)
        {
            ...
            {
                if(b != null && Vector2.Distance(b.anchoredPosition, randomPos) < 150f) 
                {
                    overlap = true;
                    break;
                }
        ...
        if(!foundPos) return; // Skip if too crowded
        ...
        // Force Anchors/Pivot to Center so (0,0) is actually the middle (Fixes Side Spawn)
        ...
        // rt.localScale = Vector3.one; // REMOVED: Auto-inherits from Prefab
